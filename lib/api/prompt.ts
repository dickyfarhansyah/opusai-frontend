...
export function fetchPrompts(user_id?: string) {
	const url = `${process.env.NEXT_PUBLIC_BASE_URL}/api/systemprompt?user_id=${user_id ? user_id : ""}`;
	return fetch(url, {
		method: "GET",
	})
		.then((res) => {
...
export function createNewPrompt({
	...
	const url = `${process.env.NEXT_PUBLIC_BASE_URL}/api/systemprompt/user?user_id=${user_id ? user_id : ""}`;
...
): Promise<UpdatePromptResponseSchema> {
	const url = `${process.env.NEXT_PUBLIC_BASE_URL}/api/systemprompt/user/${prompt_id}`;
	const body = JSON.stringify({
		title: updated_title,
