import type { ChatChunkData, ChatCompletionData, ChatErrorData, ChatStartData } from "../type/chat_message"
...
}:
{
  ...
  const formated_headers = {"Content-Type": "application/json", ...headers}
    ...
    //     console.error('Non-JSON Response', {status:response.status, contentType:contentType, body:text})
    //     data = {error: text}
    //   }
    //   if (!response.ok) {
    ...
    //   return data as ChatResponseSchema
            ...
            try {
              switch (eventType) {
                ...
                case 'chat.completion':
                  const completionData = JSON.parse(eventData) as ChatCompletionData
                ...
                case 'chat.error':
                  ...
                  if (onError) {
                    onError(`Streaming error: ${errorData.code} - ${errorData.message}`)
                  } else {
