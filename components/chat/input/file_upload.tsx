...
const ChatFileUpload = memo(function ChatFileUpload() {
	...
	const handle_file_on_change = (e: ChangeEvent<HTMLInputElement>) => {
		...
		if (!validated.success) {
			appendError(
				"Files invalid, only receives pdf, doc, docx, txt, and csv files.",
			);
			return;
		} else {
			appendFile(file);
		...
		<form>
			...
			>
				...
				<input
					...
					type="file"
					className="sr-only"
					accept="application/pdf, text/plain, application/vnd.openxmlformats-officedocument.wordprocessingml.document, application/vnd.ms-excel, text/csv, application/csv"
					onChange={handle_file_on_change}
				/>
			</InputGroupButton>
		</form>
	);
